...
class ColabProSSTUI:
    ...
    def _input_guide(self):
        return self._html(
            ...
            "be prepared using one of the two methods below. Ready-to-use "
            "input templates are provided at the bottom of this section. Use "
            "the first method when an experimental or predicted structure file "
            "is available; use the second method only when no structure file "
            "can be obtained.</p>"
            "<ol>"
            "<li><b>Sequence + structure files (recommended):</b> upload a CSV "
            "containing amino-acid sequences and the corresponding PDB/mmCIF "
            "filenames, "
            ...
            "structures. In a protein sequence, X means that the amino acid at "
            "that position is unknown. ColabProSST restores an X from the "
            "matching residue identity in the supplied structure when "
            "available. This method supports proteins up to the current ProSST "
            "input maximum of 2046 residues; the second input method supports "
            f"up to {ESMFOLD_MAX_RESIDUES} residues.</li>"
            "<li><b>Sequence only:</b> use this method only when no experimental "
            ...
            "marked for review in the audit report. ColabProSST uses ESMFold v1 "
            ...
            "and sequence + structure-file examples, plus an "
            "<code>example_structures.zip</code> containing four matching PDB "
            "files. Upload that ZIP directly with any included structure-input "
            "CSV. Example labels "
