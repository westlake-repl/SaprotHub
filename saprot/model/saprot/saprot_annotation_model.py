...
class SaprotAnnotationModel(SaprotBaseModel):
    ...
    def test_epoch_end(self, outputs):
        ...
        preds = torch.unsqueeze(torch.cat(self.test_aupr.preds, dim=-1), dim=0)
        target = torch.unsqueeze(torch.cat(self.test_aupr.target, dim=-1), dim=0).long()
    ...
    def validation_epoch_end(self, outputs):
        ...
        preds = torch.unsqueeze(torch.cat(self.valid_aupr.preds, dim=-1), dim=0)
        target = torch.unsqueeze(torch.cat(self.valid_aupr.target, dim=-1), dim=0).long()
        f1_max = count_f1_max(preds, target)
        
        log_dict = {"valid_f1_max": f1_max,
                    # "valid_loss": torch.cat(self.all_gather(outputs), dim=-1).mean(),
                    "valid_loss": torch.mean(torch.stack(outputs)),
                    # "valid_aupr": aupr
                    }
