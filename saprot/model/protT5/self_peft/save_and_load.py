...
def set_peft_model_state_dict(model, peft_model_state_dict, adapter_name="default"):
            ...
            if any(module_name in key for module_name in model.modules_to_save):
                for module_name in model.modules_to_save:
                    if module_name in key:
                        # Modify the code to make it compatible with "classifier"
    ...
    for key, value in peft_model_state_dict.items():
        assert key in model.state_dict(), f"Key '{key}' not found in model state dict"
        assert model.state_dict()[key].shape == value.shape, f"Shape mismatch for key '{key}', expected {model.state_dict()[key].shape}, got {value.shape}"
    ...
    missing_keys = model.state_dict().keys() - peft_model_state_dict.keys()
    return missing_keys
